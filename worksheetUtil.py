...
def load(file):
    ...
    print(f'opened "{ws.title}" now please give address of data, you can use excel to find ranges, students name and number must match one-to-one, means first student name saves by first student number.')
    
    students_name = get_input(lambda inp: validate_array(inp,ws),'where are the students name? (e,g: A1:A12) : ',error=[
    ...
    students_numbers = get_input(lambda inp: validate_array(inp,ws,len(students_name)), 'and where are the students number?(B1:B12) : ',error=[
        '[ERROR] Range is invalid',
